...
def extract_records(pdf_path: Path, output_dir: Path) -> list:
    ...
    Returns a list of records, one per DTC page (not per code)
    ...
    for page_num, codes in sorted(page_to_codes.items()):
        ...
        records.append({
            "schema_version": 1,                    # version of this JSON shape — bump when fields change
            "document_id": document_id,             # SHA256 fingerprint of the PDF file
            "source_file": source,                  # original PDF filename
            "page": page_num,                       # page number in the PDF
            "page_ref": page_ref_base,              # e.g. "EVB-167" — used as image filename prefix
